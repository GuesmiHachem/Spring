...
public class TableController {
    ...
    public List<TableResponseWithInvoicesDTO> getAllTables(){
        return this.service.getAllTables().stream()
                .map(el->modelMapper.map(el,TableResponseWithInvoicesDTO.class)).collect(Collectors.toList());
    }
    ...
    public TableResponseDTO createNewTable(@PathVariable("id") long id,@Valid @RequestBody TableUpdateDTO newTable){
        return this.convertToDto(service.updateTable(id,
                modelMapper.map(newTable, TableEntity.class)
        ));
    }
